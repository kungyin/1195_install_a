...
static int rtk_file_to_flash_pbc(struct t_rtkimgdesc* prtkimgdesc, const char* filepath, unsigned int soffset, unsigned int imglen, unsigned int doffset, unsigned int alignlen, unsigned int* pchecksum);
...
int rtk_burn_img(struct t_rtkimgdesc* prtkimgdesc, struct t_imgdesc* pimgdesc)
{
   int ret;
...
#else
#define EXT4_MOUNTP "/tmp"
#endif
int rtk_burn_ext4_img(struct t_rtkimgdesc* prtkimgdesc, struct t_imgdesc* pimgdesc, struct t_PARTDESC* rtk_part)
{
	int ret;
...
int rtk_burn_single_part(struct t_rtkimgdesc* prtkimgdesc, enum FWTYPE efwtype)
{
   ...
   {
      ret = rtk_burn_ext4_img(prtkimgdesc, &prtkimgdesc->fw[efwtype], rtk_part);
   }
#endif
...
int rtk_burn_cipher_key(struct t_rtkimgdesc* prtkimgdesc)
{
	...
	do_burn_efuse_key(EFUSE_KEY_RSA, sizeof(Krsa),(unsigned char *) Krsa);
	if( prtkimgdesc->secure_boot ) {
		// Krsa + Kh + Ks
		do_burn_efuse_key(EFUSE_KEY_H, sizeof(Key1), (unsigned char *)Key1);
		do_burn_efuse_key(EFUSE_KEY_S, sizeof(Key2), (unsigned char *)Key2);

		// burn the following bits:
		// secure-boot, bootcode-enc, hwsetting chk,
		do_burn_efuse_bit(EFUSE_BIT_SECURE_BOOT | EFUSE_BIT_BOOT_ENC | EFUSE_BIT_HWCHK_LATER);
	} else {
		// Krsa + Kn
		do_burn_efuse_key(EFUSE_KEY_N, sizeof(Key1), (unsigned char *)Key1);
	}
...
static int burn_fwdesc_nand(struct t_rtkimgdesc* prtkimg)
{
   ...
   if(prtkimg->fw[FW_AUDIO_CLOGO1].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_AUDIO_FILE;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.ro = 0;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.version = 0;
      ...
      //rtk_fw_head.fw_desc[rtk_fw_head.fw_count].checksum
   }

   // video customer logo1 fw_desc
   if(prtkimg->fw[FW_VIDEO_CLOGO1].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_VIDEO_FILE;
   ...
   if(prtkimg->fw[FW_AUDIO_CLOGO2].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_AUDIO_FILE;
   ...
   if(prtkimg->fw[FW_VIDEO_CLOGO2].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_VIDEO_FILE;
...
static int burn_fwdesc_emmc(struct t_rtkimgdesc* prtkimg)
{
   ...
   if(prtkimg->fw[FW_VIDEO_CLOGO1].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
	  if (strstr(prtkimg->fw[FW_VIDEO_CLOGO1].filename, "image"))
      	rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_IMAGE_FILE;
	  else
		rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_VIDEO_FILE;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.ro = 0;
   ...
   if(prtkimg->fw[FW_VIDEO_CLOGO2].flash_allo_size != 0)
   {
      fw_count++;
      rtk_fw_head.fw_count++;
	  if (strstr(prtkimg->fw[FW_VIDEO_CLOGO2].filename, "image"))
      	rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_IMAGE_FILE;
	  else
		rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.type = FW_TYPE_VIDEO_FILE;
      rtk_fw_head.fw_desc[rtk_fw_head.fw_count].v1.ro = 0;
...
int rtk_install_factory(struct t_rtkimgdesc* prtkimgdesc, bool bFlush)
{
   ...
   // if bootcode is updated, env settings of bootcode need clear too
   if (prtkimgdesc->bootcode == 1) {
      install_log("\r\n[kill %s]\r\n", ENVSETTINGS_FILENAME);
      snprintf(cmd, sizeof(cmd), "rm -f %s", ENVSETTINGS_FILENAME);
      if((ret = rtk_command(cmd, __LINE__, __FILE__, 1)) < 0)
         install_debug("kill %s fail!\r\n", ENVSETTINGS_FILENAME);
   }
...
static int rtk_burn_bootcode_part(struct t_rtkimgdesc* prtkimgdesc, char *path, unsigned int offset, unsigned char spare_id)
{
	...
	for (int i = 0; i < NAND_BOOT_BACKUP_COUNT; i++)
	{
		...
		for( unsigned j=0; j<num_of_block; j++ ) {
...
int rtk_burn_bootcode_emmc(struct t_rtkimgdesc* prtkimgdesc)
{
   ...
   char cmd[128] = {0}, path[128] = {0};
   //unsigned int bootcode_len, rescue_img_size;
   //char rescue_file_name[128] = {0};
   ...
   else
   {
      ...
      //---------[extern-param section]---------//
      install_info("\r\n[Burn extern_param]\r\n");
	  extern_param.secure_mode = prtkimgdesc->secure_boot;
	  ...
	  if (rescue_img_size != 0) {
      	extern_param.rescue_img_part0_saddr = (start/prtkimgdesc->mtd_erasesize);
		extern_param.rescue_img_addr = start/prtkimgdesc->mtd_erasesize;
      ...
      if(prtkimgdesc->ignore_native_rescue == 1)
      {
		 extern_param.rescue_img_part0_saddr = 0;
      ...
      ret = rtk_ptr_to_flash(&extern_param, len, prtkimgdesc->mtdblock_path, start);
      if(ret < 0)
      {
      ...
      // verify
      if(verify_flag) {
         ret = rtk_ptr_verify(prtkimgdesc->mtdblock_path
            , start
            , (char *)&extern_param
            , 0
            , len
...
static int rtk_file_to_flash_pbc(struct t_rtkimgdesc* prtkimgdesc, const char* filepath, unsigned int soffset, unsigned int imglen, unsigned int doffset, unsigned int alignlen, unsigned int* pchecksum)
{
   if(prtkimgdesc->ignore_native_rescue == 0)
   {
	...
	if (prtkimgdesc->flash_type == MTD_EMMC)
   		...
   		return rtk_file_to_mtd(filepath, soffset, imglen, prtkimgdesc, doffset, alignlen, pchecksum);
}
