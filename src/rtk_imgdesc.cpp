   ...
   switch (prtkimg->install_avfile_count) {
      case 3:
         ...
         if (prtkimg->fw[FW_AUDIO_CLOGO2].img_size != 0) {
            //audio clogo2 size not 0
            if (prtkimg->install_avfile_audio_size == 0) {
               //install_avfile_audio_size set as 0
               prtkimg->fw[FW_AUDIO_CLOGO2].flash_allo_size = SIZE_ALIGN_BOUNDARY_MORE(prtkimg->fw[FW_AUDIO_CLOGO2].img_size, prtkimg->mtd_erasesize);
            }
            else {
               ...
               if (prtkimg->fw[FW_AUDIO_CLOGO2].flash_allo_size < prtkimg->fw[FW_AUDIO_CLOGO2].img_size) {
                  install_fail("install_avfile_audio_size %dbytes is smaller than AUDIO CLOGO2 size %dbytes\r\n", prtkimg->install_avfile_audio_size*1024, prtkimg->fw[FW_AUDIO_CLOGO2].img_size);
                  return -1;
               }
            }
         } else {
            //audio clogo2 size 0
            //ridiculous bug from bootcdoe, Clogo2 need to program same as Dlogo
         ...
         if (prtkimg->fw[FW_VIDEO_CLOGO2].img_size != 0) {
            //video clogo2 size not 0
            if (prtkimg->install_avfile_video_size == 0) {
               //install_avfile_video_size set as 0
               prtkimg->fw[FW_VIDEO_CLOGO2].flash_allo_size = SIZE_ALIGN_BOUNDARY_MORE(prtkimg->fw[FW_VIDEO_CLOGO2].img_size, prtkimg->mtd_erasesize);
            }
            else {
               ...
               if (prtkimg->fw[FW_VIDEO_CLOGO2].flash_allo_size < prtkimg->fw[FW_VIDEO_CLOGO2].img_size) {
                  install_fail("install_avfile_video_size %dbytes is smaller than VIDEO CLOGO2 size %dbytes\r\n", prtkimg->install_avfile_video_size*1024, prtkimg->fw[FW_VIDEO_CLOGO2].img_size);
                  return -1;
               }
            }
         } else {
      ...
      case 2:
         ...
         if (prtkimg->fw[FW_AUDIO_CLOGO1].img_size != 0) {
            ...
            if (prtkimg->install_avfile_audio_size == 0) {
               //install_avfile_audio_size set as 0
               prtkimg->fw[FW_AUDIO_CLOGO1].flash_allo_size = SIZE_ALIGN_BOUNDARY_MORE(prtkimg->fw[FW_AUDIO_CLOGO1].img_size, prtkimg->mtd_erasesize);
            }
            else {
               ...
               if (prtkimg->fw[FW_AUDIO_CLOGO1].flash_allo_size < prtkimg->fw[FW_AUDIO_CLOGO1].img_size) {
                  install_fail("install_avfile_audio_size %dbytes is smaller than AUDIO CLOGO1 size %dbytes\r\n", prtkimg->install_avfile_audio_size*1024, prtkimg->fw[FW_AUDIO_CLOGO1].img_size);
                  return -1;
               }
            }
         } else {
         ...
         if (prtkimg->fw[FW_VIDEO_CLOGO1].img_size != 0) {
            //video clogo1 size not 0
            if (prtkimg->install_avfile_video_size == 0) {
               //install_avfile_video_size set as 0
               prtkimg->fw[FW_VIDEO_CLOGO1].flash_allo_size = SIZE_ALIGN_BOUNDARY_MORE(prtkimg->fw[FW_VIDEO_CLOGO1].img_size, prtkimg->mtd_erasesize);
            }
            else {
               ...
               if (prtkimg->fw[FW_VIDEO_CLOGO1].flash_allo_size < prtkimg->fw[FW_VIDEO_CLOGO1].img_size) {
                  install_fail("install_avfile_video_size %dbytes is smaller than VIDEO CLOGO1 size %dbytes\r\n", prtkimg->install_avfile_video_size*1024, prtkimg->fw[FW_VIDEO_CLOGO1].img_size);
                  return -1;
               }
            }
         } else {
...
int fill_rtkimgdesc_layout(struct t_rtkimgdesc* prtkimg)
{
   ...
   else if (prtkimg->flash_type == MTD_EMMC)
   {
      ...
      prtkimg->fw[FW_RESCUE_DT].flash_allo_size = _32K_BYTE;
      prtkimg->fw[FW_KERNEL_DT].flash_allo_size = _32K_BYTE;
...
#endif
          prtkimg->total_alloc_size += prtkimg->fw[FWTYPE(_sort[efwtype])].flash_allo_size;
...
int fill_rtkimgdesc_file(struct t_tarheader* pthead, unsigned int tarfile_offset, struct t_rtkimgdesc* prtkimg)
{
   ...
   /*
   if(strstr(pthead->filename, "rescue"))
   {
      pimgdesc = &prtkimg->fw[FW_RESCUE];
   ...
   if(strstr(pthead->filename, "srootfs.img"))
   {
      pimgdesc = &prtkimg->fw[P_SROOTFS];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   ...
   if(strstr(pthead->filename, "install_a") && (strlen(pthead->filename) == strlen("install_a")))
   {
      pimgdesc = &prtkimg->install_a;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   ...
   if(strstr(pthead->filename, "video_firmware.install.bin"))
   {
      pimgdesc = &prtkimg->video_firmware;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "video_firmware.install.bin");
      return 0;
   }

   if(strstr(pthead->filename, "uEGBK.mbf"))
   {
      pimgdesc = &prtkimg->uEGBK;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "uEGBK.mbf");
      return 0;
   }

   if(strstr(pthead->filename, "font.ttf"))
   {
      pimgdesc = &prtkimg->font;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "font.ttf");
      return 0;
   }

   if(0 == strcmp(pthead->filename, "configuration.xml"))
   {
      install_log("Got configuration.xml\r\n");
      pimgdesc = &prtkimg->configuration;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "configuration.xml");
      return 0;
   }
   */
   ...
   if(strstr(pthead->filename, "fw_98c"))
   {
      pimgdesc = &prtkimg->fw_98c;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, pthead->filename);
      return 0;
...
#if 0
   if(strstr(pthead->filename, "busybox"))
   {
      pimgdesc = &prtkimg->busybox;
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", "busybox");
      return 0;
   }

	// the following files (power on audio/video) should be handled in
	// tk_load_config() -> add_firmware().
	// they should be defined in config.txt, as follows:
	// fw = videoFile package4/bootfile.image 0xdeaddead
   if(strstr(pthead->filename, "bootfile2.audio"))
   {
      pimgdesc = &prtkimg->fw[FW_AUDIO_CLOGO2];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   }
   if(strstr(pthead->filename, "bootfile1.audio"))
   {
      pimgdesc = &prtkimg->fw[FW_AUDIO_CLOGO1];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   }
   if(strstr(pthead->filename, "bootfile.audio"))
   {
      pimgdesc = &prtkimg->fw[FW_AUDIO_BOOTFILE];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   ...
   if(strstr(pthead->filename, "bootfile2.video") || strstr(pthead->filename, "bootfile2.image"))
   {
      pimgdesc = &prtkimg->fw[FW_VIDEO_CLOGO2];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   }
   if(strstr(pthead->filename, "bootfile1.video") || strstr(pthead->filename, "bootfile1.image"))
   {
      pimgdesc = &prtkimg->fw[FW_VIDEO_CLOGO1];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
   }
   if(strstr(pthead->filename, "bootfile.video") || strstr(pthead->filename, "bootfile.image"))
   {
      pimgdesc = &prtkimg->fw[FW_VIDEO_BOOTFILE];
      pimgdesc->tarfile_offset = tarfile_offset;
      pimgdesc->img_size = octalStringToInt(pthead->filesize, FILESIZELEN);
      sprintf(pimgdesc->filename, "%s", pthead->filename);
      return 0;
